...
We have seen a few ways to define the quality of our test cases.
With structural testing we used for example line coverage, branch coverage and MC/DC.
And for model based testing we looked at transition coverage or path coverage.
These are all **adequacy criteria** that measure how much of the program is exercised by the tests.

These criteria on themselves are not enough to determine the quality of the test cases.
We can exercise a lot of the system, while testing very little.
The coverages will look great, but our test suite could be useless.

{% include example-begin.html %}
We have a simple class with one method.
...
Now we write the following to tests:
...
If we look at branch coverage this will give $$100\%$$.
We exercise both branches given by the condition.
It will even give $$100\%$$ statement coverage, as all the statements are executed at least once.

You probably noticed that something is missing in these tests: The assertions!
We can make any change in the production code and the tests will always pass.
{% include example-end.html %}

## Fault Detection Capability

Another adequacy criterion is the **Fault detection capability**.
This indicates the test's capability to find faults in the code.
...
Using this criterion we can indicate the quality of our test suite in a better way than with just the coverages.
...
For a test to be adequate according to this criterion, it has to have a meaningful test oracle (meaningful assertions).

The fault detection capability as a test adequacy criterion is the fundamental idea behind mutation testing.
With mutation testing we change small parts of the code and check if the tests find the introduced fault.

{% include example-begin.html %}
In the previous example we made a test suite that was not adequate at all according to the fault detection capability.
As there are no assertions, the tests will never find any faults in the code.
...
The tests with assertions check if the result of the method is what we expect: We have a test oracle now.
To see how the values in a test case influence the fault detection capability we create two tests, where the denominator is not 0.
...
For the fault detection capability we want to see if the tests catch faults that could be in the code.
...
The introduced fault is the multiplication, when computing the quotient; this should be a division.
If we run our tests with the wrong code, we see that the test with the values 1 and 1 still passes, but the other test fails.
...
In this case, the input values and test oracle of the `testGetValuesDifferent` test are better.
{% include example-end.html %}
...
If we have a good test suite, at least one of the tests will fail on this changed code.
With this procedure we get a sense of the fault error capability of our test suite.
...
**Competent Programmar Hypothesis (CPH)**\\
Here we assume that the program is written by a competent programmer.
More importantly this means: Given a certain specification, the programmer creates a program that is either correct, or it differs from a correct program by a combination of simple errors.
...
With these two hypotheses we can determine the size that the mutants should have.
Realistically, following the competent programmer hypothesis, the faults in actual code will be small.
...
So we can indeed keep the mutant size small.
It is close to reality and if our test suite works well for these mutants, it will also work well for more complex faults.

## Terminology

To talk about mutation testing in-depth we need to define some terms first.
...
- **Syntactic Change**: A small *change*. Such a small change should make the code still valid, i.e. is can still compile and run.
...
{% include example-begin.html %}
First we have a `Fraction` class with a method `invert`.
...
You can see that we have two tests for some corner cases, that throw an exception, and two more normal tests.
...
If we would execute the test suite on this mutant, the `testInvertNegative` will fail: `result.getFloat()` would be positive instead of negative.

Another mistake could be made in line 1.
With boundary analysis we saw that it is important to test the boundaries because of off-by-one errors.
We can make a syntactic change by introducing such an off-by-on error.
Instead of `numerator == 0`, in our new mutant we say `numerator == 1`:
...
Manually writing the mutations of our programs takes a lot of time and we probably only think of the cases that are already tested.
So, like with test execution, we want to automate this process.
...
This means that if the generator sees a statement in the code, that corresponds to the grammatic rule of the operator (e.g. `a + b`), then the mutation operator specifies how to change this statement with a syntactic change (turning it into `a - b` for example).

We distinguish two categories of mutation operators:

- **Real fault based operators**: Operators that apply changes, which are very similar to defects seen in the past for the same kind of code. So when someone creates such an operator they will look at common mistakes made by programmers with similar code.
- **Language-specific operators**: Mutations that are made specifically for a certain programming language. For example changes to inheritance in Java, we cannot simply make the same changes in another language. Another example would be changes regarding pointers in C.
...
Each time we first show the original code, and then the mutant that could be given by the mutant operator.
...
For Java, there are a lot of language-specific operators.
We can for example change the inheritance, remove an overriding method or changing some declaration types.
...
We have explained the concept of mutation testing and its automation.
Now we can look at how we can use mutation testing in practice.

We want to use mutation testing to determine the quality of our test suite.
...
When performing mutation testing, we get killed mutants and mutants that are still alive.
By counting the amount of each of these mutant groups we can give a value to the quality of our test suite.\\
We define the **Mutation Score**:

$$\text{Mutation score } = \frac{\text{#killed mutants}}{\text{#mutants}}$$

Computing this mutation score is called mutation analysis.
More formally:\\
**Mutation Analysis** means assessing the quality of a test suite, by computing its mutation score.

When the mutation score is low, we might want to change the existing or add new test cases.
...
Then it is indeed useful to improve the test suite.
Still there is a small difference between the two.
It is important to notice this difference, as you might want to divide work between the two concepts.
...
If the mutant behaves like the normal code, it will give the same output as the original program for any given input.
This makes it impossible to detect the mutant in the tests.
...
The original code decremented `index`, which started at 10, and breaked from the loop when index would be equal to 0.
...
A lot of research done in software engineering tries to give some insight to this problem.
...
Of course, mutation testing is not free.
...
The first observation is that a test case can never kill a mutant if it does not cover the statement that changed.
Based on this observation, we only have to run the test cases that cover the changed statement.
This reduces the amount of test cases run and with that the execution time.
...
The exact number of test cases killing it does not really matter.

A second oberservation made is that mutants generated by the same operator and injected at the same location are likely to be coupled to the same type of fault.
...
It is then highly likely that if the test suite kills one of the mutants, it will also kill the other.
The heuristic that follows from this observation is to run the test suite against less mutants.
Obviously when we run the test suite less times, the mutation testing will take less time.
The simplest way of selecting a subset of mutants is random sampling.
...
As PIT is a mutation testing tool, it generated the mutants and runs the test suites against these mutants.
...
Finally, you can also see more detailed results in the source code and check which individual mutants were kept alive.
